...
def download_chunks(url, output_dir, image):
  ...
  """
    try:
        ...
        if e.code == 500:
            print
            'error code 500: file doesnt exist'
            pass
        else:
            print
...
def find_valid_scene(ref_time, prow, sat, delta=16):
    ...
    while attempts < 5:
        ...
        if not scene_found:
            ...
            print
            'Looking for version/station combination....'
            ...
            if not scene_found:
                ...
                print
                'No scene, moving {} days ahead to {}'.format(delta, datetime.strftime(ref_time, '%Y%j'))
                attempts += 1
...
def assemble_scene_id_list(ref_time, prow, sat, end_date, delta=16):
    ...
    while ref_time < end_date:
        ...
        print
        'add scene: {}, for {}'.format(scene_str,
...
def down_usgs_by_list(scene_list, output_dir, usgs_creds_txt):
    ...
    for product in scene_list:
        ...
        if not os.path.isdir(scene_dir):
            ...
            print
            'image: {}'.format(os.path.join(scene_dir, tgz_file))
